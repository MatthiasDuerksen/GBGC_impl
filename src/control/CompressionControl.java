...
public class CompressionControl {
     ...
     */
    private final List<Tuple<Graph, List<DigramRule>>> allGraphs = new ArrayList<>();
...
//     * @return
//     */
//    private Node getNodeInEdgeAndOccurrence(Edge edge, DigramOccurrence occ) {
...
//     */
//    private int getNewEquivClassForEdge(List<Tuple<Integer, Integer>> tuples, Edge edge, Node endOrStartNode) {
...
//                Edge oldEdge = (Edge) allEdges.get(i);
...
//                            Edge newEdge = new Edge(nodeMiddle, oldEdge.getEquivalenceClass(nodeMiddle), newNode, equivClassMidNode);
//                            allEdges.remove(i);
...
//                    newEdge = new Edge(newNode, newEquivClass, oldEdge.getEndnode(), oldEdge.getEquivalenceClass(oldEdge.getEndnode()));
//                } else if (nodeOcc.equals(oldEdge.getEndnode())) {
//                    int newEquivClass = getNewEquivClassForEdge(tuples, oldEdge, oldEdge.getEndnode());
//                    newEdge = new Edge(oldEdge.getStartnode(), oldEdge.getEquivalenceClass(oldEdge.getStartnode()),
...
//     */
//    private void updateDigramList(Graph graph, Digram appliedDigram) {
...
//     */
//    public LinkedList<String> getAllDuplicatedLabelsLabels(Graph graph) {
...
//     */
//    public static void checkAndAddEdgeToDigram(Edge edge, BasicDigramList_old digramList, List<Digram> appliedDigrams) {
//        BasicDigram_old digram = digramList.getDigram(edge, appliedDigrams);
...
//     */
//    private void pruning(Graph graph, LinkedList<Digram> digrams) {
//        LinkedList<Digram> tmpDigrams = (LinkedList<Digram>) digrams.clone();
//        for (Digram digram : tmpDigrams) {
//            String nt = digram.getNonterminal();
//            Tuple<BasicDigram_old, Integer> occurrences = getNTOccurrencesDigram(nt);
...
//     */
//    private Graph edgeOptimization(Graph graph) {
...
//        for (Map.Entry<Integer, Node> entry : graph.getAllNodes().entrySet()) {
//            Node node = entry.getValue();
//            LinkedList<Edge> incidentEdges = graph.getAllIncidentEdges(node);
...
//            graph.add(new Edge(new Node[]{incommingEdge.getStartnode()}, new Node[]{outcommingEdge.getEndnode()}, node.getLabel()));
//
...
//     */
//    private boolean checkCorrectDirection(LinkedList<Edge> edges, Node node) {
//        Edge enteringEdge = null;
//        Edge leavingEdge = null;
//        for (Edge edge : edges) {
...
//     */
//    private Tuple<BasicDigram_old, Integer> getNTOccurrencesDigram(String nt) {
//        BasicDigram_old currentDigram = null;
//        int counter = 0;
//        for (Digram digram : basicDigramList.getAllActiveDigrams()) {
//            if (digram.getNumOccurrences(nt) > 0) {
//                currentDigram = (BasicDigram_old) digram;
...
//
     ...
     */
    private void addCurrentGraph(Graph graph, LinkedList<DigramRule> digrams) {
        allGraphs.add(new Tuple<>(graph.clone(), (LinkedList<DigramRule>) digrams.clone()));
    }
